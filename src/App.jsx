...
function buildTicketText(order) {
  const lines = [];
  lines.push(`Commande #${order.ticket}`);
  lines.push(order.time || "");
  lines.push("--------------------------------");
  order.items.forEach((it) => {
    lines.push(`${it.qty}x ${it.name}${it.sizeLabel ? ` (${it.sizeLabel})` : ""}`);
    const sub = lineSubtitle(it);
    if (sub) lines.push(`   ${sub}`);
  });
  lines.push("--------------------------------");
  lines.push(`Total : ${money(order.total)}`);
  lines.push("A regler au comptoir");
  return lines.join("\n");
}
...
function printOrder(order) {
  ...
  try {
    window.AndroidPrinter.printTicket(buildTicketText(order));
  } catch (e) {
  ...
  if (step === "payment") {
    ...
    return (
      <div className="screen">
        ...
        <div className="modal-section">
          <span className="modal-label">Email — pour être prévenu quand c'est prêt (obligatoire)</span>
          <input
            className="input"
            type="email"
          ...
          {email.trim().length > 0 && !emailValid && (
            <p className="track-error">Adresse email invalide.</p>
          )}
        </div>
        ...
        <button
          ...
          disabled={!emailValid}
          style={!emailValid ? { opacity: 0.4, cursor: "not-allowed" } : undefined}
          onClick={placeOrder}
        >
...
function PrinterSettingsModal({ onClose }) {
  ...
  const testPrint = () => {
    if (!hasNativePrinter()) return;
    window.AndroidPrinter.printTicket("Test d'impression\nO'Delices\n--------------------------------\nSi tu lis ce ticket,\nl'imprimante est bien configuree.");
  };
