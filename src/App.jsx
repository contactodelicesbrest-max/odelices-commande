...
import { Plus, Minus, ShoppingBag, X, Check, Clock, ArrowLeft, Bell, Settings, Printer } from "lucide-react";
...
function buildTicketSegments(order) {
  const segs = [];
  const push = (text, opts = {}) => segs.push({ text, bold: !!opts.bold, big: !!opts.big, center: !!opts.center });
  ...
  order.items.forEach((it) => {
    push(`${it.qty}x ${it.name}${it.sizeLabel ? ` (${it.sizeLabel})` : ""}`, { bold: true });
    if (it.removed?.length) push(`  Sans ${it.removed.join(", ")}`);
    if (it.meats?.length) push(`  Viande : ${it.meats.join(", ")}`);
    if (it.bread) push(`  ${it.bread}`);
    if (it.extraChoice) push(`  ${it.extraChoice}`);
    if (it.sauces?.length) push(`  Sauce${it.sauces.length > 1 ? "s" : ""} : ${it.sauces.join(", ")}`);
    if (it.supplements?.length) push(`  Suppl. : ${it.supplements.map((s) => (s.qty > 1 ? `${s.name} x${s.qty}` : s.name)).join(", ")}`);
  });

  push("--------------------------------");
  push(`Client : ${order.clientName || "-"}`, { bold: true });
  push(`TOTAL : ${money(order.total)}`, { bold: true, big: true });
  if (order.amountGiven != null) {
    push(`Donne : ${money(order.amountGiven)}`);
    push(`Rendu : ${money(Math.max(0, order.amountGiven - order.total))}`, { bold: true });
  }
