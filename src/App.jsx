...
function buildTicketSegments(order) {
  ...
  if (order.amountGiven != null) {
    push(`Donne : ${money(order.amountGiven)}`, { tall: true });
    push(`Rendu : ${money(Math.max(0, order.amountGiven - order.total))}`, { bold: true, tall: true });
  }
  ...
  return (
    <div className="ticket">
      ...
      {order.amountGiven != null && (
        <>
          <div className="ticket-item-top"><span>Donné</span><span>{money(order.amountGiven)}</span></div>
          <div className="ticket-item-top" style={{ fontWeight: 700 }}><span>Rendu</span><span>{money(Math.max(0, order.amountGiven - order.total))}</span></div>
        </>
      )}
      ...
      <div className="screen">
        ...
        <div className="modal-section">
          ...
          {changeDue !== null && !isNaN(changeDue) && changeDue >= 0 && (
            <p className="modal-desc" style={{ margin: 0 }}>Rendu de monnaie : <strong>{money(changeDue)}</strong></p>
          )}
          {changeDue !== null && !isNaN(changeDue) && changeDue < 0 && (
    ...
    <div className="app-root">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .item-card { text-align: left; background: var(--panel); border: 1px solid var(--border); border-radius: 14px; padding: 16px; cursor: pointer; color: var(--text); transition: border-color .15s ease; }
        .item-card:hover { border-color: var(--fire); }
        .item-card-top { display: flex; justify-content: space-between; font-weight: 700; font-size: 19px; gap: 8px; }
        .item-price { color: var(--gold); white-space: nowrap; }
        .item-desc { color: var(--muted); font-size: 14.5px; margin: 6px 0 12px; }
        .item-cta { display: inline-flex; align-items: center; gap: 4px; font-size: 13.5px; font-weight: 700; color: var(--fire); }
