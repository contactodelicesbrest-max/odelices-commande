...
function buildTicketSegments(order) {
  ...
  order.items.forEach((it) => {
    ...
    if (it.removed?.length) push(`  Sans ${it.removed.join(", ")}`, { bold: true, tall: true });
    if (it.meats?.length) push(`  Viande : ${it.meats.join(", ")}`, { bold: true, tall: true });
    if (it.bread) push(`  ${it.bread}`, { bold: true, tall: true });
    if (it.extraChoice) push(`  ${it.extraChoice}`, { bold: true, tall: true });
...
function printOrder(order) {
  ...
  try {
    window.AndroidPrinter.printTicket(JSON.stringify(buildTicketSegments(order)));
  } catch (e) {
